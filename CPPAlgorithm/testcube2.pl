...
print "Rubik's Cube Solver Test, version 3.0.1 (C) Marek Karcz 2015.\n";
print "All rights reserved.\n";
print "Usage:\n";
print "   $0 [num_tests [method_number [scramble_steps | file_name]]]\n";
print "Where:\n";
print "   num_tests      - number of tests to perform (default: 100)\n";
print "   method_number  - algorithm number accepted by program (default: 4)\n";
print "   scramble_steps - steps to scramble cube (0 or default: random)\n";
print "   file_name      - if file name is used, test set will be read\n";
print "                    from the plain text file, which must contain\n";
print "                    cube states in color format, no spaces, one\n";
print "                    cube per line of text\n";
...
while ($tests > 0) {
    ...
    my $cmd = "rubik -a $methnum -s $scrsteps -b -c -C";
    $input = `$cmd`;
    @lines = split /\n/, $input;
    ...
    if ($issolved == 1) {
	   ...
	   my $cm2 = "rubik -a $methnum -e $cubedef -r $steps -b -c";
	   my $cubinp = `$cm2`;
	   my @inplin = split /\n/, $cubinp;
	   $issolved = 0;
	   foreach (@inplin) {
